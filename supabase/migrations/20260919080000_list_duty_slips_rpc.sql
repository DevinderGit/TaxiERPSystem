-- TAXI-801 — Public-schema RPC for the Duty Slip list page.
--
-- Same PostgREST custom-schema workaround as every other M2..M7 RPC.
-- RLS on operations.duty_slips (from TAXI-110) still applies —
-- tenant_isolation + write_requires_operator.
...
AS $$
  SELECT
    ...
    ds.opening_km,
    ds.closing_km,
    -- total_km is GENERATED ALWAYS AS … STORED (TAXI-104) but we expose
    -- it here as a regular column for clarity. Postgres returns the
    -- stored computed value when you SELECT it.
    ds.total_km,
    ds.duty_start_dt,
    ds.duty_end_dt,
    -- total_hours computed in hours (fractional). NULL when duty_end_dt
    -- is not yet filled (open duty slip).
    CASE
